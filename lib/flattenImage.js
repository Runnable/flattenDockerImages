...
function flattenImage (imageName, repo, layerCount, cb) {
  ...
  async.series([
    ...
    function cloneLayers (cb) {
      ...
      async.eachSeries(history, function cloneLayer (layer, cb) {
        copyLayer(join('/var/lib/docker/graph/', layer, '/layer/'),
          join(__dirname, tempLayerDest), tempLayerDest, function (err, listToReDelete) {
            if (err) {
              cb(err);
            } else if (listToReDelete.length) {
              cb(new Error('redelete: ' + listToReDelete.map(function (file) {
                return file.replace(/.+\/layer\//, join('/var/lib/docker/graph/', layer, '/layer/'));
              }).join(' ')));
            } else {
              cb();
            }
    ...
    function pushImage (cb) {
      cb();
      exec('docker push ' + repo, function (err) {
        if (err) {
          console.error(err);
        }
      });
