...
function doAUFSRecursiveCopy (srcDir, dstDir, tempLayerDest, cb) {
  ...
  async.series([
    function getList (cb) {
      getListToDelete(srcDir, dstDir, listToDelete, whitelist, cb);
    },
    ...
    if (err) {
      ...
      cb(new Error('whitelist: ' + JSON.stingify(whitelist)));
    }
  });
}

// recursive list maker
function getListToDelete (root, dstDir, listToDelete, whitelist, cb) {
  var files;
    ...
    function processFiles (cb) {
      async.eachSeries(files, function eachFile (file, cb) {
        ...
        if (/^\.wh\./.test(file)) {
          ...
          whitelist[fullPath] = false;
          fs.stat(fullPath, function (err, stats) {
            if (err) {
              cb();
            } else if (stats.isDirectory()) {
              var dstDirFullPath = join(dstDir, file);
              getListToDelete(fullPath, dstDirFullPath, listToDelete, whitelist, cb);
            } else {
              cb();
            }
          });
